...
- 标准 RSS 2.0 订阅：通过 `/rss.xml` 获取最近论文与已有解读，支持 `topic_id` 和 `limit` 参数。
...
阅读端页脚提供“RSS 每日论文”入口，RSS 阅读器也可直接订阅：

```text
https://你的站点/rss.xml
```

默认返回最近 50 篇论文，已有成功解读时优先展示解读，否则展示原始摘要。可以使用
`?topic_id=主题ID` 仅订阅一个主题，使用 `&limit=20` 调整为 1 至 100 篇。RSS 是公开只读
服务，仅包含论文公开元数据与已有解读，不包含笔记、标签、收藏、已读状态、模型或后台配置。
